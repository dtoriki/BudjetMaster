...
3. [Структура и форматирование кода](#3-структура-и-форматирование-кода)
   ...
   - 3.7 [Форматирование inline-комментариев](#37-форматирование-inline-комментариев)
4. [XML-документация](#4-xml-документация)
   ...
   - 4.7 [Исключения в документации интерфейсов](#47-исключения-в-документации-интерфейсов)
...
### 3.7. Форматирование inline-комментариев
...
Правило не распространяется на XML-документацию (`///`) и заголовочные комментарии тестовых файлов — у них своё соглашение (см. раздел [5.2](#52-комментарии-в-тестах)).
...
### 4.7. Исключения в документации интерфейсов

Интерфейс определяет контракт поведения, но не его реализацию. Выброс конкретных исключений — деталь реализации, а не часть контракта. Поэтому в XML-документации интерфейсов **не используй** теги `<exception>` — ни на уровне интерфейса, ни на уровне его членов.
...
Документируй исключения только в конкретных реализациях (классах) согласно правилам раздела [4.3](#43-документирование-исключений).
...
- Inline-комментарии: однострочные — `//`, многострочные — `/* ... */` с `*` на каждой строке.
...
- В интерфейсах теги `<exception>` **не используются** — только в конкретных реализациях.
